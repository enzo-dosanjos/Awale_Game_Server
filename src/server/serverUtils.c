                  ...
                  int *actualConnected)
{
    ...
    memmove(connectedClients + to_remove, connectedClients + to_remove + 1,
            (*actualConnected - to_remove - 1) * sizeof(Client *));
    /* number client - 1 */
    (*actualConnected)--;
}
...
{
    memmove(lobby + to_remove, lobby + to_remove + 1,
            (*lobby - to_remove - 1) * sizeof(SOCKET));
    (*actualLobby)--;
}
...
int addChallenge(Client *challenger, Client *challenged)
{
    ...
    {
        if (strcmp(challenger->pendingChallengesTo[i], challenged->username) ==
            0)
        {
    ...
    {
        char msg[] =
                "Error: You have reached the maximum number of pending "
                "challenges. Please wait for one to be accepted.";
        writeClient(challenger->sock, msg);
        return 0;
    }
    ...
    {
        char msg[] =
                "Error: The user you are trying to challenge has reached the "
                "maximum number of pending challenges. Please try again later.";
        writeClient(challenged->sock, msg);
        return 0;
    }

    strcpy(challenger->pendingChallengesTo[challenger->numPendingChallengesTo],
           challenged->username);
    challenger->numPendingChallengesTo++;

    strcpy(challenged->pendingChallengesFrom
...
int removeChallenge(Client *client, Client *challenged)
{
    ...
    for (int i = 0; i < client->numPendingChallengesTo; i++)
    {
        if (strcmp(client->pendingChallengesTo[i], challenged->username) == 0)
        {
            ...
            {
                strcpy(client->pendingChallengesTo[j],
                       client->pendingChallengesTo[j + 1]);
            }
    ...
    for (int i = 0; i < challenged->numPendingChallengesFrom; i++)
    {
        if (strcmp(challenged->pendingChallengesFrom[i], client->username) == 0)
        {
            ...
            {
                strcpy(challenged->pendingChallengesFrom[j],
                       challenged->pendingChallengesFrom[j + 1]);
            }
            challenged->numPendingChallengesFrom--;
            break;
        }
...
void clearSentChallenge(Client *client) { client->numPendingChallengesTo = 0; }
